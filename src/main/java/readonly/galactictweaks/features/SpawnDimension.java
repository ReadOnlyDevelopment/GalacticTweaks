...
public class SpawnDimension extends Feature implements IOrdered {
	...
	public void postInit() {
		...
		for(CelestialBody body : GalaxyRegistry.getAllReachableBodies()) {
			if(body.getName().equalsIgnoreCase(idName)) {
				spawnDim = Objects.requireNonNull(GalaxyRegistry.getPlanetOrMoonFromTranslationkey(body.getTranslatedName()), err + idName);
			}
		}

		GCTLog.debug("Spawn Dimension Set: " + spawnDim.getTranslatedName());
	}
