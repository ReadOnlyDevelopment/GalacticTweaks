...
public class NBTFormatter {
	public static final String[] TAG_NAMES = new String[] { "TAG_End", "TAG_Byte", "TAG_Short", "TAG_Int", "TAG_Long",
	...
	private static void getPrettyFormattedLine(List<String> lines, NBTBase nbt, String name, int depth) {
		...
		try {
			...
			GalacticTweaks.LOG.warn("Error while printing NBT data");
		}
		...
		case Constants.NBT.TAG_BYTE_ARRAY:
			...
			// For short arrays, print one defaultValue per line, it is easier to read
			if (len <= 16) {
		...
		case Constants.NBT.TAG_LIST:
			...
			lines.add(pre + getTagDescription(nbt.getId(), name) + " (" + tagCount + " entries of "
					+ getTagName(tagType) + " (" + tagType + "))");
			lines.add(pre + "{");
