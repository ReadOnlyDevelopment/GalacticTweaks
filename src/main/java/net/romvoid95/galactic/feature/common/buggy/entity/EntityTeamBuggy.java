...
public class EntityTeamBuggy extends Entity
		implements IInventory, IPacketReceiver, IDockable, IControllableEntity, IEntityFullSync {
	private static final DataParameter<Integer> CURRENT_DAMAGE = EntityDataManager.createKey(EntityTeamBuggy.class,
			DataSerializers.VARINT);
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.createKey(EntityTeamBuggy.class,
			DataSerializers.VARINT);
	private static final DataParameter<Integer> ROCK_DIRECTION = EntityDataManager.createKey(EntityTeamBuggy.class,
			DataSerializers.VARINT);
	public static int tankCapacity = 1000;
	public FluidTank buggyFuelTank = new FluidTank(EntityTeamBuggy.tankCapacity);
	...
	public EntityTeamBuggy(World var1) {
		super(var1);
	...
	public EntityTeamBuggy(World var1, double var2, double var4, double var6, int type) {
		this(var1);
	...
	public int getScaledFuelLevel(int i) {
		...
		return (int) (fuelLevel * i / EntityTeamBuggy.tankCapacity);
	}
	...
	public void updatePassenger(Entity passenger) {
		if (this.isPassenger(passenger)) {
			...
			final double offsetX = Math.cos(this.rotationYaw / Constants.RADIANS_TO_DEGREES_D + 114.8) * f;
			final double offsetZ = Math.sin(this.rotationYaw / Constants.RADIANS_TO_DEGREES_D + 114.8) * f;
	...
	public void onUpdate() {
		...
		if (this.world.isRemote && !FMLClientHandler.instance().getClient().player
				.equals(this.world.getClosestPlayerToEntity(this, -1))) {
			double x;
			double y;
			double var12;
			double z;
		...
		if (this.inWater && this.speed > 0.2D) {
			this.world.playSound(null, (float) this.posX, (float) this.posY, (float) this.posZ,
					SoundEvents.ENTITY_GENERIC_BURN, SoundCategory.NEUTRAL, 0.5F,
		...
		if (this.world.isRemote) {
			...
			GalacticraftCore.packetPipeline.sendToAllAround(new PacketEntityUpdate(this),
					new TargetPoint(GCCoreUtil.getDimensionID(this.world), this.posX, this.posY, this.posZ, 50.0D));
	...
	public boolean pressKey(int key) {
		if (this.world.isRemote && (key == 6 || key == 8 || key == 9)) {
			GalacticraftCore.packetPipeline
					.sendToServer(new PacketSimple(PacketSimple.EnumSimplePacket.S_CONTROL_ENTITY,
							GCCoreUtil.getDimensionID(this.world), new Object[] { key }));
			return true;
		}
	...
	public EnumCargoLoadingState addCargo(ItemStack stack, boolean doAdd) {
		...
		for (count = 0; count < this.stacks.size(); count++) {
			...
			if (stackAt != null && stackAt.getItem() == stack.getItem()
					...
					&& stackAt.getCount() < stackAt.getMaxStackSize()) {
