package net.rom.gctweaks.gc.features.sprfeature.command;
			...
			if (race != null) {
				...
				if (!sender.getName().equalsIgnoreCase(owner)) {
					...
					if (!race.getPlayerNames().contains(player.getName())) {
						sender.sendMessage(new TextComponentTranslation("gctweaks.leaverace.playerleave", race
								.getTeamName()));
					}  else {
						
					}
				} else {
					sender.sendMessage(new TextComponentTranslation("gctweaks.leaverace.leader"));
				}
			} else {
				sender.sendMessage(new TextComponentTranslation("gctweaks.leaverace.noteam"));
			}
		}
		else {
			sender.sendMessage(new TextComponentTranslation("gctweaks.leaverace.noarg"));
			sender.sendMessage(new TextComponentTranslation("gctweaks.leaverace.useage", this.getUsage(sender)));
		}
	}
