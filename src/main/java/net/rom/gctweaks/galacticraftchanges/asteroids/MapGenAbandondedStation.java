...
public class MapGenAbandondedStation extends MapGenStructure {
	private static boolean initialized;

	static {
		try {
			MapGenAbandondedStation.initiateStructures();
		}
		catch (Throwable e) {
	...
	public MapGenAbandondedStation() {}

	public static void initiateStructures () throws Throwable {
		if (!MapGenAbandondedStation.initialized) {
			MapGenStructureIO.registerStructure(MapGenAbandondedStation.Start.class, "AbandonedBase");
		...
		MapGenAbandondedStation.initialized = true;
	}
	...
	protected boolean canSpawnStructureAtCoords (int chunkX, int chunkZ) {
		long dungeonPos = MapGenDungeon
				.getDungeonPosForCoords(this.world, chunkX, chunkZ, ((IGalacticraftWorldProvider) this.world.provider)
						.getDungeonSpacing());
	...
	protected StructureStart getStructureStart (int chunkX, int chunkZ) {
		BlockVec3 asteroid = ((WorldProviderAsteroids) this.world.provider)
				.getClosestAsteroidXZ((chunkX << 4) + 8, 0, (chunkZ << 4) + 8, false);
		if (asteroid == null) {
			return new MapGenAbandondedStation.Start(this.world, this.rand, (chunkX << 4) + 8, (chunkZ << 4)
					+ 8, 15, new BaseConfiguration(148, this.rand));
		}
		return new MapGenAbandondedStation.Start(this.world, this.rand, asteroid.x, asteroid.z, asteroid.sideDoneBits
				- 5, new BaseConfiguration(asteroid.y - 10, this.rand));
	}
