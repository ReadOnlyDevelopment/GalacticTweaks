...
public class ChunkProviderGCTAsteroids extends ChunkProviderBase {
	...
	public static List<SpecialAsteroidBlock> coreBlocks  = new ArrayList<SpecialAsteroidBlock>();
	public static List<SpecialAsteroidBlock> shellBlocks = new ArrayList<SpecialAsteroidBlock>();

	public static List<Block> asteroidBlocks = new ArrayList<Block>();
	...
	private static final int CHUNK_SIZE_Y = 512;
	...
	private static final int MAX_ASTEROID_Y = ChunkProviderGCTAsteroids.CHUNK_SIZE_Y - 48;
	...
	private final MapGenAbandondedStation dungeonGenerator = new MapGenAbandondedStation();

	public ChunkProviderGCTAsteroids(World par1World, long par2, boolean par4) {
		...
		this.asteroidSkewX.amplitude  = ChunkProviderGCTAsteroids.MAX_ASTEROID_SKEW;
		this.asteroidSkewX.frequencyX = 0.005F;

		this.asteroidSkewY            = new Gradient(this.rand.nextLong(), 1, 1);
		this.asteroidSkewY.amplitude  = ChunkProviderGCTAsteroids.MAX_ASTEROID_SKEW;
		this.asteroidSkewY.frequencyY = 0.005F;

		this.asteroidSkewZ            = new Gradient(this.rand.nextLong(), 1, 1);
		this.asteroidSkewZ.amplitude  = ChunkProviderGCTAsteroids.MAX_ASTEROID_SKEW;
		...
		for (SpecialAsteroidBlock core : coreBlocks) {
			...
			GalacticTweaks.logger.info("## Registering [" + core.block.getUnlocalizedName() + "] to Asteroid Cores");
		}
		...
		for (SpecialAsteroidBlock shell : shellBlocks) {
			...
			GalacticTweaks.logger.info("## Registering [" + shell.block.getUnlocalizedName() + "] as Asteroid Shells");
		}
	}
	...
	public void generateTerrain (int chunkX, int chunkZ, ChunkPrimer primer, boolean flagDataOnly) {
		...
		final int    asteroidChance = ChunkProviderGCTAsteroids.ASTEROID_CHANCE;
		final int    rangeY         = ChunkProviderGCTAsteroids.MAX_ASTEROID_Y
				- ChunkProviderGCTAsteroids.MIN_ASTEROID_Y;
		final int    rangeSize      = ChunkProviderGCTAsteroids.MAX_ASTEROID_RADIUS
				- ChunkProviderGCTAsteroids.MIN_ASTEROID_RADIUS;

		for (int i = chunkX - 3; i < chunkX + 3; i++) {
			int minX = i * 16;
			int maxX = minX + ChunkProviderGCTAsteroids.CHUNK_SIZE_X;
			for (int k = chunkZ - 3; k < chunkZ + 3; k++) {
				int minZ = k * 16;
				int maxZ = minZ + ChunkProviderGCTAsteroids.CHUNK_SIZE_Z;

				for (int x = minX; x < maxX; x += 2) {
					for (int z = minZ; z < maxZ; z += 2) {
						if (this.randFromPointPos(x, z) < (this.asteroidDensity.getNoise(x, z) + .4) / asteroidChance) {
							random.setSeed(x + z * 3067);
							int y    = random.nextInt(rangeY) + ChunkProviderGCTAsteroids.MIN_ASTEROID_Y;
							int size = random.nextInt(rangeSize) + ChunkProviderGCTAsteroids.MIN_ASTEROID_RADIUS;
	...
	private void generateAsteroid (Random rand, int asteroidX, int asteroidY, int asteroidZ, int chunkX, int chunkZ, int size, ChunkPrimer primer, boolean flagDataOnly) {
		...
		if (rand.nextInt(ChunkProviderGCTAsteroids.ASTEROID_SHELL_CHANCE) == 0) {
			shell = this.shellHandler.getBlock(rand, size);
		}

		boolean     isHollow   = false;
		final float hollowSize = rand.nextFloat()
				* (ChunkProviderGCTAsteroids.MAX_HOLLOW_SIZE - ChunkProviderGCTAsteroids.MIN_HOLLOW_SIZE)
				+ ChunkProviderGCTAsteroids.MIN_HOLLOW_SIZE;
		if (rand.nextInt(ChunkProviderGCTAsteroids.HOLLOW_CHANCE) == 0
				&& size >= ChunkProviderGCTAsteroids.MIN_RADIUS_FOR_HOLLOW) {
			isHollow = true;
			shell    = new SpecialAsteroidBlock(AsteroidBlocks.blockDenseIce, (byte) 0, 1, .15);
		}

		// Add to the list of asteroids for external use
		((WorldProviderSpaceRocks) this.world.provider)
				.addAsteroid(asteroidX, asteroidY, asteroidZ, size, isHollow ? -1 : core.index);

		final int xMin  = this
				.clamp(Math.max(chunkX, asteroidX - size - ChunkProviderGCTAsteroids.MAX_ASTEROID_SKEW - 2)
						- chunkX, 0, 16);
		final int zMin  = this
				.clamp(Math.max(chunkZ, asteroidZ - size - ChunkProviderGCTAsteroids.MAX_ASTEROID_SKEW - 2)
						- chunkZ, 0, 16);
		final int yMin  = asteroidY - size - ChunkProviderGCTAsteroids.MAX_ASTEROID_SKEW - 2;
		final int yMax  = asteroidY + size + ChunkProviderGCTAsteroids.MAX_ASTEROID_SKEW + 2;
		final int xMax  = this
				.clamp(Math.min(chunkX + 16, asteroidX + size + ChunkProviderGCTAsteroids.MAX_ASTEROID_SKEW + 2)
						- chunkX, 0, 16);
		final int zMax  = this
				.clamp(Math.min(chunkZ + 16, asteroidZ + size + ChunkProviderGCTAsteroids.MAX_ASTEROID_SKEW + 2)
						- chunkZ, 0, 16);
					...
					if (distance <= 1) {
						if (isHollow && distance <= hollowSize) {
							...
							if (rand.nextBoolean()) {
								...
								primer.setBlockState(x, y, z, ChunkProviderGCTAsteroids
										.getRandomElement(asteroidBlocks).getDefaultState());
							}
						}
				...
				for (int z = zMin; z < zMax; z++) {
					...
					for (int y = yMin; y < yMax; y++) {
						...
						if (distance <= 1) {
							...
							if (Blocks.AIR == stateAbove.getBlock() && (state.getBlock() == ChunkProviderGCTAsteroids
									.getRandomElement(asteroidBlocks).getDefaultState()
									|| state.getBlock() == GRASS)) {
	...
	public Chunk generateChunk (int par1, int par2) {
		...
		if (this.world.provider instanceof WorldProviderSpaceRocks
				&& ((WorldProviderSpaceRocks) this.world.provider).checkHasAsteroids()) {
	...
	public void populate (int chunkX, int chunkZ) {
		int x = chunkX << 4;
		int z = chunkZ << 4;
		if (!ChunkProviderGCTAsteroids.chunksDone.add(new BlockVec3(x, 0, z))) {
			return;
		}
		...
		if (this.rand.nextBoolean()) {
			...
			double numOfBlocks = this.clamp(this.randFromPoint(chunkX, chunkZ), .4, 1)
					* ChunkProviderGCTAsteroids.MAX_BLOCKS_PER_CHUNK * density
					+ ChunkProviderGCTAsteroids.MIN_BLOCKS_PER_CHUNK;
			int    y0          = this.rand.nextInt(2);
			Block  block;
			int    yRange      = ChunkProviderGCTAsteroids.MAX_ASTEROID_Y - ChunkProviderGCTAsteroids.MIN_ASTEROID_Y;
			x += 4;
			z += 4;

			for (int i = 0; i < numOfBlocks; i++) {
				int y = this.rand.nextInt(yRange) + ChunkProviderGCTAsteroids.MIN_ASTEROID_Y;

				// 50:50 chance vertically as well
				if (y0 == (y / 16) % 2) {
					int px = x + this.rand.nextInt(ChunkProviderGCTAsteroids.CHUNK_SIZE_X);
					int pz = z + this.rand.nextInt(ChunkProviderGCTAsteroids.CHUNK_SIZE_Z);

					if (ChunkProviderGCTAsteroids.asteroidBlocks.contains(AsteroidBlocks.blockBasic)) {
						...
						if (this.rand.nextInt(ILMENITE_CHANCE) == 0) {
							...
							if (ConfigManagerAsteroids.disableIlmeniteGen) {
								...
								ChunkProviderGCTAsteroids.asteroidBlocks
										.add(AsteroidBlocks.blockBasic.getStateFromMeta(meta).getBlock());
								continue;
							}
						}
						...
						block = ChunkProviderGCTAsteroids.getRandomElement(asteroidBlocks);
		...
		if (!this.largeAsteroids.isEmpty()) {
			for (AsteroidData asteroidIndex : new ArrayList<AsteroidData>(this.largeAsteroids)) {
				...
				if (ConfigManagerCore.challengeAsteroidPopulation
						|| rand.nextInt(ChunkProviderGCTAsteroids.TREE_CHANCE) == 0) {
					...
					for (int tries = 0; tries < 5; tries++) {
						...
						if (wg.generate(world, rand, new BlockPos(i, this.getTerrainHeightAt(i - x, k
				...
				if (!treesdone || rand.nextInt(ChunkProviderGCTAsteroids.TALL_GRASS_CHANCE) == 0) {
					int i = rand.nextInt(16) + x + 8;
					int k = rand.nextInt(16) + z + 8;
					new WorldGenTallGrass(GRASS_TYPE).generate(world, rand, new BlockPos(i, this.getTerrainHeightAt(i
							- x, k - z, sizeYArray, xMin, zMin, zSize, asteroidY, asteroidSize), k));
				}
				if (rand.nextInt(ChunkProviderGCTAsteroids.FLOWER_CHANCE) == 0) {
					int   i     = rand.nextInt(16) + x + 8;
					int   k     = rand.nextInt(16) + z + 8;
					int[] types = new int[] { 2, 4, 5, 7 };
					new WorldGenFlowers(this.FLOWER, EnumFlowerType
							.getType(BlockFlower.EnumFlowerColor.RED, types[rand.nextInt(types.length)]))
									.generate(world, rand, new BlockPos(i, this.getTerrainHeightAt(i - x, k
											- z, sizeYArray, xMin, zMin, zSize, asteroidY, asteroidSize), k));
				}
				if (rand.nextInt(ChunkProviderGCTAsteroids.LAVA_CHANCE) == 0) {
					int i = rand.nextInt(16) + x + 8;
					int k = rand.nextInt(16) + z + 8;
					new WorldGenLakes(this.LAVA).generate(world, rand, new BlockPos(i, this.getTerrainHeightAt(i - x, k
							- z, sizeYArray, xMin, zMin, zSize, asteroidY, asteroidSize), k));
				}
				if (rand.nextInt(ChunkProviderGCTAsteroids.WATER_CHANCE) == 0) {
					int i = rand.nextInt(16) + x + 8;
					int k = rand.nextInt(16) + z + 8;
					new WorldGenLakes(this.WATER).generate(world, rand, new BlockPos(i, this.getTerrainHeightAt(i - x, k
							- z, sizeYArray, xMin, zMin, zSize, asteroidY, asteroidSize), k));
				}
			}
		}
				...
				for (int z = a.zMinArray; z < a.zMax; z++) {
					for (int y = yMin; y < yMax; y++) {
						if (chunk.getBlockState(x - 1, y, z).getBlock() instanceof BlockAir
							...
							if (count > 12)
								count = 12;
							chunk.setBlockState(new BlockPos(x - 1, y, z), GCBlocks.brightAir
									.getStateFromMeta(13 - count));
	...
	private boolean isLargeAsteroidAt0 (int x0, int z0) {
		for (int x = x0; x < x0 + ChunkProviderGCTAsteroids.CHUNK_SIZE_X; x += 2) {
			for (int z = z0; z < z0 + ChunkProviderGCTAsteroids.CHUNK_SIZE_Z; z += 2) {
				if ((Math.abs(this.randFromPoint(x, z)) < (this.asteroidDensity.getNoise(x, z) + .4)
						/ ChunkProviderGCTAsteroids.ASTEROID_CHANCE)) {
					return true;
				}
			}
		}
